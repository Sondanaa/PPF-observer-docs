...
def filter_transform(mag_data, output_filter, R=None, B=None, 
    ...
    if output_filter == 'R':
        if r and i != None:
            try:
                ...
                raise KeyError('key', str(r), 'not found in mag data for r mags')
            try:
                i_mags = mag_data[i]
            except:
                raise KeyError('key', str(i), 'not found in mag data for i mags')
            A = -0.0107
            B = 0.0050
            C = -0.2689
            D = -0.1540
            ...
            raise KeyError('arguemnts r and i must be defined to transform to I filter')
    ...
    if output_filter == 'I':
        if r and i != None:
            ...
            raise KeyError('arguments r and i must be defined to transform to I filter')
    ...
    if output_filter == 'B':
        if r and g != None:
            ...
            try:
                ...
                raise KeyError('key', str(i), 'not found in mag data for g mags')
            A = 0.2628
            B = -0.7952
            C = 1.0544
            D = 0.02684
            ...
            raise KeyError('arguemnts r and g must be defined to transform to B filter')
    ...
    if output_filter == 'V':
        if r and g != None:
            ...
            raise KeyError('arguments r and g must be defined to transform to B filter')
    else:
